...
| ~~Two components named "model gateway"~~ | **Answered by the Tech Lead 2026-09-24, and neither page named it:** the approved route for all model traffic is **Portkey**, the Bain-standard AI gateway. Portkey does not appear anywhere in the mirror — not in Technology Choices §4 (which names "API Management GenAI gateway"), not in Andromeda (which names its own "Model gateway"). Both pages are stale on this point and should be corrected to name Portkey, or to state how it relates to APIM/Andromeda if it sits in front of either |
| **Red (Restricted) and Confidential (Orange) are being treated as one thing** | The [VCC overview meeting](../confluence/oi30/meeting-summaries/vcc-overview-meeting-19696746551.md) writes "red (confidential)" throughout, as if the two were synonyms. Per Bain's actual classification scheme (surfaced 2026-09-24 via a reviewer on the production-approval questionnaire), **Red = Restricted** and **Confidential = Orange** are two distinct, non-overlapping levels with different requirements. Which one actually applies to Partner uploads has never been ratified — the meeting note's phrasing assumed an equivalence that doesn't exist. Blocks item 1 of `requests/2026-09-23-production-approval-confirmations.md` |
