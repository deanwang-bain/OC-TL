...
Andromeda capability

 |
|---|---|
|

Running LLMs (e.g. Claude Sonnet/Opus)

 |

Model gateway — governed access, no data leaving Bain
...
BGE-M3 + reranker, served on Andromeda GPU compute
...
Managed Kubernetes — networking, scaling, TLS, isolation
...
Workload identity, SSO via Bain IdP (OIDC/SAML)
...
Shared observability via OpenTelemetry standard
...
Andromeda programmatic/API access confirmed

 |

Bain (Felipe / Sandeep/ Michelle)

 |

Meeting held; Sandeep follow-up with Andromeda PM pending

 |
|

CoE validation gate — Andromeda integration scenario
