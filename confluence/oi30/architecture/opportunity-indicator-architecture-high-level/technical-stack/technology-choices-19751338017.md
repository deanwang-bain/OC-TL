...
Azure managed services first, custom build only where genuinely warranted
...
StatusNeo writes and operates the code
...
An Azure managed service, configured, with integration code only
...
An existing Bain or third-party system consumed under licence
...
Does it do the job the capability model asks for
...
Is it managed, and if not, is the custom build warranted
...
What does it cost to change our mind in eighteen months
...
Who runs it, and with what expertise
...
Static assets from Azure Front Door CDN
...
Radix with a Bain-token theme layer
...
Interactive tables inside the product
...
Ephemeral. The user inspects, filters, expands and exports. Nothing persists into the content model

 |
|

Exhibits that travel into the deck
...
An Exhibit is a specification bound to data. It serialises, versions with the case, and renders identically into HTML, PDF and presentation
...
Headless Chromium for HTML and PDF, OOXML writer for presentation
...
Source metric, derived calculation, parameter, aggregation, adjustment, normalisation
...
Arithmetic, comparison, conditional, a fixed function library, named references to semantic metrics
...
Loops, recursion, imports, file or network access, arbitrary function definition
...
Looks like a spreadsheet formula, because the people authoring it live in spreadsheets
...
Deterministic. Memoised. Topologically ordered with cycle detection at compile time
...
Immutable blob with a legal hold policy
...
OAuth2 client credentials, separate audience
...
Entra external OAuth to Snowflake
...
Private endpoints, Private DNS, public access disabled
...
Frontend, services, calculation engine, data transformations, infrastructure

 |

Exact. Same input, same output, every time
...
Agents, model output, retrieval, ranking

 |

Invariants and distributions. Never exact strings
...
Component behaviour, state transitions, tool-call handling
...
Stage progression, drill-down, assistant actions, export
...
Deck rendering fidelity across HTML, PDF and presentation
...
Schemathesis against OpenAPI

 |

The implementation matches the schema, including cases nobody wrote a test for
...
The calculation contract cannot break a consumer silently
...
Testcontainers: SQL Server, Redis, Azurite

 |

Real substrates, not mocks. DuckDB is in process and needs no container
...
Determinism, commutativity where claimed, cycle rejection, unit and currency invariants
...
pytest with pinned cases

 |

A known set of inputs produces a known set of figures, byte for byte
...
A stored trace plus pinned definition versions reproduces the original figure exactly
...
Uniqueness, referential integrity, accepted values, freshness, at bronze to silver
...
PSRule for Azure, Checkov

 |

Private endpoints present, public access disabled, no inline secret
...
Terraform plan or Bicep what-if in the pipeline

 |

No unreviewed change reaches an environment
...
Every claim has an evidence binding. Every figure has a trace. Every adjustment names its authority. No opportunity references a metric that does not exist. These are cheap, absolute, and fail the run rather than the deployment
...
Groundedness of extracted claims against their source passage. Faithfulness of narrative to the analysis it summarises. Verification intensity scales with the effort budget
...
Custom harness against a golden dataset

 |

Peer set overlap against a reference set above a threshold. Opportunity ranking stability. Claim extraction recall. Aggregate scores, never individual strings
...
Before a release and on a sample

 |

Structured review by an analyst

 |

Whether the output is any good. Nothing above answers this and nothing will
...
Dependabot with alerts as blocking
...
Microsoft Defender for Cloud, Trivy in the pipeline
...
Every release, and on any change to the tool contract
...
Under 30 minutes wall clock

 |

Scenario test against a reference case
...
Measured against the proof of concept baseline of 10 to 12 peers in 3.5 to 4 minutes
...
First partial result visible within 30 seconds of run start

 |

Playwright with timing assertions
...
No production document ever reaches a test environment

 |

Uploads may carry MNPI or PII. There is no safe way to hold them outside production

 |
|

Synthetic companies for functional tests

 |

Named fictitiously, with a full metric profile, so tests do not depend on a live source

 |
|

One pinned CapIQ snapshot for the golden dataset

 |

Real data, frozen, licensed for internal use, small enough to version

 |
|

Source responses recorded and replayed

 |

Integration tests must not consume live API quota, which is a constrained resource

 |
|

Warehouse tests run against a masked subset
...
Unit, contract, lint, CodeQL, secret scan, dbt compile
...
Integration with Testcontainers, engine golden fixtures, property tests, image scan
...
End to end, accessibility, visual regression, infrastructure policy
...
DAST, load, trace replay against the full corpus, offline model regression
...
AI red team, human review sample, full performance scenario
...
Includes the domain services, which are custom by definition
...
Model gateway, orchestration platform, batch orchestrator
...
Of which only three realise a capability outright
...
Governed, versioned, scope-resolved deterministic calculation with a derivation trace as a first-class output. No managed service does this. It is also the one component Bain can reuse, which is the argument for building it as a service on day one
...
Business-authored definitions with versioning that never overwrites, scope binding with inheritance, and dry-run validation. Adjacent products exist for feature stores and metric layers; none carries the scope hierarchy or the authoring lifecycle this needs
...
Two claim producers, one evidence contract, licensing enforcement in one place, and drill-down from a rendered claim to a raw input in one traversal. This is the product's defensibility and it has no off-the-shelf shape
...
**Composition and template engine**

 |

Bain supplies brand assets; the engine that turns a format-neutral content model into HTML, PDF and presentation is ours. Confirmed by Bain
...
Neither orchestration substrate has a native external quota concept. A known residual cost, not a preference
...
SQL ledger tables with digests to immutable blob

 |

A hand-rolled hash chain, and the burden of proving it correct
...
Native SharePoint indexing

 |

An embedding pipeline, a second copy of every document, and a sync problem
...
Graph validation, tracing, and effort budget propagation, all rebuilt
...
A parser per format, maintained forever
...
Azure AI Content Safety at the gateway

 |

Redaction logic duplicated in every service that touches a model
...
Nothing sensible. This one was never a real question
...
Guided workspace and assistant

 |

Plus self-service authoring surface, branch and compare
...
Plus Durable Functions and Logic Apps
...
Real-time fetch per run

 |

Monthly batch for CapIQ, VCC and AURA; run-time for the rest
...
Parquet in SharePoint under a case

 |

ADLS Gen2 plus a Snowflake warehouse
...
Platinum layer serving business meaning
...
Inside a case version

 |

Across the warehouse, in silver
...
Engineering-mediated through source control

 |

Self-service for a Bain steward
...
Plus other Bain teams via the platform API and platinum layer
...
Foundry evaluations at runtime

 |

Plus adversarial judges and offline regression
...
Vega-Lite, Apache Echarts for exhibits

 |

High cost. The exhibit object shape depends on it
...
Moderate. A rewrite of the shell, not the app
...
Container Apps, not AKS

 |

Moderate. Containers move; the environment does not
...
Two runtimes split on IO versus compute

 |

High cost to unify later
...
Closed expression grammar for calculation definitions

 |

Very high. Every authored definition depends on it
...
Low. Adding one later is additive
...
dbt for bronze to silver

 |

Moderate. Transformations are portable SQL
...
Separate APIM product for platform consumers

 |

Low, and much higher if deferred
...
AG-Grid for tables, Vega-Lite for exhibits, with a rule between them

 |

Low for AG-Grid, high for Vega-Lite
...
Zvec as a run-scoped in-memory vector cache, never a store

 |

Low. It holds nothing
...
Two testing tracks, deterministic and probabilistic, asserted differently

 |

High. It shapes what the suite even measures
...
Offline model regression blocks a release, not a merge

 |

Low. A pipeline setting
...
**Foundry network isolation**

 |

Some features, tracing and workflow agents among them, do not yet fully support network isolation. This decides whether the AI zone can be fully private or needs a compensating control. Largest unresolved item in the set
...
CapIQ rate limits and per-call cost

 |

The rate limiting component's design, and whether real-time fetch survives at 2300 users
...
Whether anything needs a second availability zone
...
Digest publication cadence, and storage sizing
...
Role set per case and what each role gates

 |

Entitlement enforcement logic at the BFF
...
MNPI enforcement in product or by policy

 |

Whether per-document access control is MVP or later
...
Case state machine, archival and deletion semantics

 |

Retention and disposal behaviour
...
The golden dataset for offline evaluation

 |

Loop 3 of the probabilistic track. It needs Bain analyst time to build reference peer sets, and nothing substitutes for that
